...
def main(queue):
    while True:
        ...
        # (sarebbero le misure da usare per calcolare in modo dinamico x1_ratio e y1_ratio)
        h, w, _ = image.shape
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if label == 'Right':
                    ...
                    queue.put(x*x1_ratio, y*y1_ratio)
                elif label == 'Left':
                    # Calcola la distanza tra la punta del pollice e la punta del mignolo
                    # per determinare apertura/chiusura
                    ...
                    global left_hand_closed, click_counter
                    if distance < 0.1:
